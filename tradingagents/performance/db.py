...
def get_performance_data() -> Dict[str, Any]:
    ...
    score_summary = _build_score_summary(records)
    return {"records": records, "summary": summary, "score_summary": score_summary}


# Score bucket boundaries (inclusive lower bound), labeled low-to-high.
...
def _build_score_summary(records: List[Dict]) -> Dict:
    """Aggregate win rates and average returns by score bucket × horizon.

    Unlike signal buckets, score buckets have no inherent bullish/bearish
    direction, so "win" simply means a positive raw return.
    """
    from collections import defaultdict

    returns_by_bucket: Dict[str, Dict[str, list]] = defaultdict(lambda: defaultdict(list))
    alphas_by_bucket: Dict[str, Dict[str, list]] = defaultdict(lambda: defaultdict(list))
    for rec in records:
        if rec["score"] is None:
            continue
        bucket = _score_bucket(rec["score"])
        if bucket is None:
            continue
        for hz, snap in rec["snapshots"].items():
            returns_by_bucket[bucket][hz].append(snap["raw_return"])
            alphas_by_bucket[bucket][hz].append(snap["alpha_return"])

    summary = {}
    for label, _, _ in SCORE_BUCKETS:
        horizons = returns_by_bucket.get(label)
        if not horizons:
            continue
        summary[label] = {}
        for hz, returns in horizons.items():
            ...
            alphas = alphas_by_bucket[label][hz]
            summary[label][hz] = {
                "count":      len(returns),
